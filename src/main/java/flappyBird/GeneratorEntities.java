...
public class GeneratorEntities {
    ...
    public Powerup generateRandomPowerUp(){
        ...
        return switch (number){
            case 0 -> new Faster(new Position(width, 0), new Dimension(1,1), 1, new Diagonal());
            case 1 -> new Health(new Position(width, random.nextInt(height)+1), new Dimension(1,1), 1, new Horizontal());
            case 2 -> new Invincible(new Position(width,0), new Dimension(1,1), 1, new Vertical());
            case 3 -> new Life(new Position(width,0), new Dimension(1,1), 1, new Vertical());
